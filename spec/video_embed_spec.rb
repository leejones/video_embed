...
describe VideoEmbed do
  describe 'YouTube' do
    it 'returns embed html from a url' do
      ...
      expect(video_embed).to eql(%Q{<iframe width="560" height="315" src="http://www.youtube.com/embed/4Z3r9X8OahA?rel=0" frameborder="0" allowfullscreen></iframe>})
    end

    it 'returns embed html from a short url' do
      video_embed = VideoEmbed.embed('http://youtu.be/4Z3r9X8OahA')
      expect(video_embed).to include('http://www.youtube.com/embed/4Z3r9X8OahA?rel=0')
    end
  ...
  describe 'Vimeo' do
    it 'returns embed html from a url' do
      ...
      expect(video_embed).to eql(%Q{<iframe src="http://player.vimeo.com/video/11040425?title=0&amp;byline=0&amp;portrait=0" width="560" height="315" frameborder="0" webkitAllowFullScreen mozallowfullscreen allowFullScreen></iframe>})
    end

    it 'returns embed html from a mobile url' do
      video_embed = VideoEmbed.embed('http://vimeo.com/m/11040425')
      expect(video_embed).to include('http://player.vimeo.com/video/11040425?title=0&amp;byline=0&amp;portrait=0')
    end
